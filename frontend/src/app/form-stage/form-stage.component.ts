...
export class FormStageComponent implements OnInit, AfterViewInit {
  ...
  addStage() {
    let stage: Stage = {
      ...
      description: this.Description.value, // Assurez-vous que la description est définie ici
      dateDebut: this.DateDebut.value,
      dateFin: this.DateFin.value,
      stagiaire: this.Stagiaire.value, // Assurez-vous que l'objet stagiaire complet est assigné
      projet: this.Projet.value, // Assurez-vous que l'objet projet complet est assigné
      profile: this.fileInput.nativeElement.files[0]?.name,
      evaluations: []
    };
  
    this.stageService.postStage(stage).subscribe((res) => {
  ...
  editStage(event: number) {
    this.stages.forEach((val) => {
      if (val.id === event) {
        this.setForm(val);
      }
    });
    this.removeStage(event);
    this.addStageButton.nativeElement.click();
  }
